...
const links = [
  { to: "/clinical/ward", end: true, label: "Clinical Ward" },
];

export function ClinicalNavRail() {
  const user = useAuthStore((s) => s.user);
  const logout = useAuthStore((s) => s.logout);

  return (
    <aside className="flex w-56 shrink-0 flex-col border-r border-calm-200 bg-slate-50/90">
      <div className="border-b border-calm-200 px-4 py-5">
        <p className="text-[10px] font-semibold uppercase tracking-widest text-vitality-600">
          Clinical Command
        </p>
